    ...
    public Node cloneGraphBFS(Node node) {
        ...
        while(!queue.isEmpty()) {
            ...
            for(int i=0; i<size; i++) {
                ...
                Node cloneNode = visited.get(curr);
                for(Node neighbour: curr.neighbors) {
                    if(visited.containsKey(neighbour)) {
                        cloneNode.neighbors.add(visited.get(neighbour));
                    }else {
                        Node neighbourCloneNode = new Node(neighbour.val, new ArrayList<>());
                        cloneNode.neighbors.add(neighbourCloneNode);
                        visited.put(neighbour, neighbourCloneNode);
